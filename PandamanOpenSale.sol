...
// File: contracts/token/PandamanOpenSale.sol
...
/// @title PandamanOpenSale Contract

contract PandamanOpenSale is Pausable,Ownable {
    using SafeMath for uint256;
    using Address for address;

    struct condition {
        uint256 price;          //_panda per bnb
    ...
    address payable public _teamWallet = 0x0b65a80Aed7f4f7695Cc9d74F4C2ce3c1EB13416;
    
    /// IERC20 compilant pandaman token contact instance
    IERC20 public _panda = IERC20(0xf3cC6800F62589E020AAA19feaE6011b40015042);
        ...
        public
    {
        ...
        setCondition(1,255000000000 ,20*1e18, now,          51000000000000*1e18);
        setCondition(2,200000000000 ,10 *1e18, now + 7 days, 60000000000000*1e18);
        setCondition(3,140000000000 ,10 *1e18, now + 7 days, 49000000000000*1e18);
        setCondition(4,100000000000 ,10 *1e18, now + 7 days, 35000000000000*1e18);
        setCondition(5,100000000000 ,10 *1e18, now + 14 days, 15000000000000*1e18);
    ...
    */
    ...
    /**
     * @dev If anybody sends Ether directly to this  contract, consider he is getting Pandaman token
     */
    ...
    /// @dev Exchange msg.value ether to Pandaman for account recepient
    function buyPanda() payable public 
        whenNotPaused
    {
        // Do not allow contracts to game the system
        address receipient = msg.sender;
        uint8 stage = conditionCheck(receipient);
        if(stage==_whiteListStage5 ){  
    ...
    /// @dev Buy Pandaman token normally
    function doBuy(address receipient, uint8 stage) internal {
        ...
        if (costValue > 0) {
            ...
            _panda.transfer(msg.sender, getTokens);   
